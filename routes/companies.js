  ...
  try {
    if (allFields) {
      ...
      if (Object.keys(filteredFields).length > 0) {
        ...
        if (filteredFields['minEmployees']) {
          min = filteredFields['minEmployees'];
        }
        if (filteredFields['maxEmployees']) {
          max = filteredFields['maxEmployees'];
        }
        if (min > max) {
          throw new BadRequestError("minEmployees can't be greater than maxEmployees", 400);
        }
      }
    }
