...
describe("GET /companies?queryStringArgs", function () {
  let resp;
  ...
  test("combo filter", async function () {
    resp = await request(app).get("/companies?name=c&maxEmployees=2");
    expect(resp.body).toEqual({
      companies:
          [
            {
  ...
  test("min higher than max error", async function () {
    resp = await request(app).get("/companies?minEmployees=3&maxEmployees=2");
    expect(resp.status).toBe(400);
    expect(resp.body.error.message).toEqual("minEmployees can't be greater than maxEmployees");
  });

  test('irrelevant query params are ignored', async function() {
    resp = await request(app).get("/companies?name=c&location=Montana&maxEmployees=2");
    expect(resp.body).toEqual({
      companies:
          [
            {
